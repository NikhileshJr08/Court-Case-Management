...
class Case(FlaskForm):
    party_name = StringField("Party name:" , validators=[InputRequired() , Length = 20 ])
...
class Hearing(FlaskForm):
    ...
    judge =  StringField("Judge:" , validators=[InputRequired() , Length = 20 ])
    start = TimeField("Start time",validators=InputRequired())
    end = TimeField("End time",validators=InputRequired())
    location = StringField("Location :" , validators=[InputRequired() , Length = 20 ])
